<?php
	...
	use ScheduleControl\Logic\DBRoom;
	...
	AJAXRequest::Proccess(function() {
		...
		$room = new DBRoom(AJAXRequest::GetIntParameter("room"));

		if (!$room->IsValid())
			AJAXRequest::ClientError(0, "Некорректный кабинет");

		$unavs = array();

		foreach (Collections::GetCollection("NotAvailableRooms")->GetAllLastChanges(array("Room" => $room->GetID())) as $unav)
		{
		...
		$collection = Collections::GetCollection("NotAvailableRooms");
		$nextid = 0;
		...
		if ($data["period"])
		{
			...
			if (($end->getTimestamp() - $first->getTimestamp()) / 60 / 60 < 1)
				AJAXRequest::ClientError(4, "Разница между началом и окончанием первой недоступности должна быть не менее 1 часа");
